...
use crate::patcher::patch_structs::{Modification, PatchPlan};

pub fn create_patch_plan(
    old_manifest: Manifest,
    new_manifest: Manifest,
) -> Result<PatchPlan, io::Error> {
    ...
    let old_version = old_manifest.game_ver;
    let new_version = new_manifest.game_ver;
    ...
    for file in &new_manifest.files {
        ...
        match old_file_lookup.get(&file.file_path) {
            Some(existing_file) => {
                if file.file_hash == existing_file.file_hash {
                    continue;
                }
